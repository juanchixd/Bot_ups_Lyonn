...
tb = telebot.TeleBot('AAAAAAA:BBBBBBBBBBBBBBBBBBBBBBBBB')
...
def send_welcome(message):
    tb.reply_to(
        message, "Welcome to The UPS Bot, please use the commands to get information.\nSuscribe notification is WIP")
...
def reply_serv(message):
    ...
    sprun = subprocess.run(
        ['service', 'apcupsd', 'status'], stdout=subprocess.PIPE)
...
def dummy_response(message):
    chat_id = message.from_user
    tb.send_message(str(
        chat_id.id), "This Bot is configured to use with commands, please do not use text")
